...
import io.streaming.warship.config.KafkaProperties;
...
class WarshipKafkaResourceIT {
    ...
    void setup() {
        ...
        WarshipKafkaResource kafkaResource = new WarshipKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/warship-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/warship-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
